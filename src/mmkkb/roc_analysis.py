"""
ROC analysis module for MMK Knowledge Base.
Provides ROC curve analysis, logistic regression modeling, and performance metrics
for biomarker combinations in experiments.
"""
import sqlite3
import json
import numpy as np
...
@dataclass
class ROCAnalysis:
    """ROC Analysis model representing an analysis run."""
    name: str
    description: str
    experiment_id: int
...
@dataclass
class ROCModel:
    """ROC Model representing a specific biomarker combination model."""
    analysis_id: int
...
class ROCAnalysisDatabase:
    """Database operations for ROC analysis."""
    ...
    def init_database(self):
        """Initialize database with ROC analysis tables."""
        with sqlite3.connect(self.db_path) as conn:
            ...
            # ROC metrics table
            conn.execute("""
                CREATE TABLE IF NOT EXISTS roc_metrics (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    ...
                    ppv REAL NOT NULL,
                    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (model_id) REFERENCES roc_models (id) ON DELETE CASCADE
                )
            """)
            
            # ROC curve points table
            conn.execute("""
                CREATE TABLE IF NOT EXISTS roc_curve_points (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    model_id INTEGER NOT NULL,
                    fpr REAL NOT NULL,
                    tpr REAL NOT NULL,
            ...
            # Create indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_roc_analyses_experiment_id ON roc_analyses (experiment_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_roc_models_analysis_id ON roc_models (analysis_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_roc_metrics_model_id ON roc_metrics (model_id)")
    ...
    def create_roc_analysis(self, analysis: ROCAnalysis) -> ROCAnalysis:
        ...
        with self._get_connection() as conn:
            ...
            row = conn.execute(
                "SELECT created_at FROM roc_analyses WHERE id = ?", 
                (analysis.id,)
            ).fetchone()
            if row:
    ...
    def get_roc_analysis_by_id(self, analysis_id: int) -> Optional[ROCAnalysis]:
        ...
        with self._get_connection() as conn:
            ...
            row = conn.execute(
                "SELECT * FROM roc_analyses WHERE id = ?", (analysis_id,)
            ).fetchone()
    ...
    def list_roc_analyses(self, experiment_id: Optional[int] = None) -> List[ROCAnalysis]:
        """List ROC analyses, optionally filtered by experiment."""
        analyses = []
        with self._get_connection() as conn:
            conn.row_factory = sqlite3.Row
            
            if experiment_id:
                query = "SELECT * FROM roc_analyses WHERE experiment_id = ? ORDER BY created_at DESC"
                rows = conn.execute(query, (experiment_id,)).fetchall()
            else:
                query = "SELECT * FROM roc_analyses ORDER BY created_at DESC"
                rows = conn.execute(query).fetchall()
            
            for row in rows:
                analyses.append(ROCAnalysis(
                    id=row["id"],
                    name=row["name"],
                    description=row["description"],
                    experiment_id=row["experiment_id"],
    ...
    def get_roc_models_by_analysis(self, analysis_id: int) -> List[ROCModel]:
        ...
        with self._get_connection() as conn:
            conn.row_factory = sqlite3.Row
            rows = conn.execute(
                "SELECT * FROM roc_models WHERE analysis_id = ? ORDER BY auc DESC",
                (analysis_id,)
            ).fetchall()
            
            for row in rows:
                models.append(ROCModel(
                    id=row["id"],
    ...
    def create_roc_metrics(self, metrics: ROCMetrics) -> ROCMetrics:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("""
                INSERT INTO roc_metrics (model_id, threshold_type, threshold, sensitivity, specificity, npv, ppv)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ...
            row = conn.execute(
                "SELECT created_at FROM roc_metrics WHERE id = ?", 
                (metrics.id,)
            ).fetchone()
            if row:
    ...
    def get_roc_metrics_by_model(self, model_id: int) -> List[ROCMetrics]:
        """Get all metrics for a model."""
        metrics = []
        with self._get_connection() as conn:
            conn.row_factory = sqlite3.Row
            rows = conn.execute(
                "SELECT * FROM roc_metrics WHERE model_id = ?",
                (model_id,)
            ).fetchall()
    ...
    def create_roc_curve_points(self, points: List[ROCCurvePoint]) -> List[ROCCurvePoint]:
        ...
        with self._get_connection() as conn:
            for point in points:
                cursor = conn.execute("""
                    INSERT INTO roc_curve_points (model_id, fpr, tpr, threshold)
                    VALUES (?, ?, ?, ?)
                ...
                row = conn.execute(
                    "SELECT created_at FROM roc_curve_points WHERE id = ?", 
                    (point.id,)
                ).fetchone()
                if row:
    ...
    def get_roc_curve_points_by_model(self, model_id: int) -> List[ROCCurvePoint]:
        """Get all ROC curve points for a model."""
        points = []
        with self._get_connection() as conn:
            conn.row_factory = sqlite3.Row
            rows = conn.execute(
                "SELECT * FROM roc_curve_points WHERE model_id = ? ORDER BY fpr",
                (model_id,)
            ).fetchall()
...
class ROCAnalyzer:
    ...
    def run_roc_analysis(self, analysis: ROCAnalysis) -> Dict[str, Any]:
        """
        Run complete ROC analysis for an experiment.
        
        Args:
            analysis: ROC analysis configuration
            
        Returns:
            Dictionary with analysis results and summary statistics
        """
        # Create the analysis record
        created_analysis = self.roc_db.create_roc_analysis(analysis)
        ...
        # Generate all biomarker combinations
        biomarker_combinations = self._generate_biomarker_combinations(
            experiment_data['biomarker_versions'], 
    ...
    def _analyze_biomarker_combination(self, analysis_id: int, 
        ...
        if se_97_metrics:
            ...
            self.roc_db.create_roc_metrics(metrics)
        ...
        self.roc_db.create_roc_curve_points(roc_points)
    ...
    def generate_analysis_report(self, analysis_id: int) -> pd.DataFrame:
        """Generate a comprehensive report for ROC analysis."""
        # Get analysis info
        analysis = self.roc_db.get_roc_analysis_by_id(analysis_id)
        if not analysis:
            raise ValueError(f"Analysis {analysis_id} not found")
        
        # Get all models for this analysis
        models = self.roc_db.get_roc_models_by_analysis(analysis_id)
        ...
        for model in models:
            ...
            metrics = self.roc_db.get_roc_metrics_by_model(model.id)
